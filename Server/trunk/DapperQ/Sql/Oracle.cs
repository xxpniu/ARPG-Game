...
{
    internal class Oracle : SqlStatement
    {
        ...
        {
            return string.Join(" ", string.Format("UPDATE [{0}] SET ([{1}])=SELECT {2}", Target,
                string.Join("], [", Settor.Select(o => o.Key).ToArray()),
                string.Join(", ", Settor.Select(o => o.Value).ToArray())),
                GenFromStatement());
        }
        ...
        {
            return string.Format("SELECT 1 FROM user_ind_columns t,user_indexes i " +
                "WHERE t.index_name = i.index_name AND t.table_name=upper('{0}') AND ROWNUM<=1",
                From.First().Value);
        }

        protected override string GenGetIndexStatement()
        {
            return string.Format("SELECT t.index_name AS idx_name FROM user_ind_columns t,user_indexes i " + 
                "WHERE t.index_name = i.index_name AND t.table_name=UPPER('{0}')", 
                From.First().Value);
        }
        ...
        protected override string GenExistsStatement()
        {
            ...
            sb.AppendFormat("SELECT 1 {0} ", GenFromStatement());
            sb.Append(GenSkipOrTop());
            return sb.ToString();
        }
        ...
        {
            return string.Format("DELETE {0}", GenFromStatement());
        }
        ...
        private string GenSkipOrTop()
        {
            ...
            if (Skip > 0 && Top > 0)
                sb.AppendFormat("{0}", Top > 0 ? string.Format("ROWNUM>{0} AND ROWNUM<={1}", Skip, Top) : string.Empty);
            else if (Skip > 0 && Top < 1)
                sb.AppendFormat("{0}", Top > 0 ? string.Format("ROWNUM>{0}", Skip) : string.Empty);
            else if (Skip < 1 && Top > 0)
                sb.AppendFormat("{0}", Top > 0 ? string.Format("ROWNUM<={0}", Top) : string.Empty);
        ...
        protected override string DatetimeFormat(string sql)
        {
            ...
            sql = regex.Replace(sql, "TO_DATE('$1','yyyy/mm/dd hh24:mi:ss')");
