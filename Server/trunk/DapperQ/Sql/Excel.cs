...
{
    internal class Excel : Access
    {
        public override string ToString(Type type)
        {
            var sql = base.ToString(type);
            Regex regex = new Regex(@"(UPDATE|INSERT INTO|FROM|DELETE|JOIN|\()\s+\[([^\[\$\]]+)\]", RegexOptions.Multiline | RegexOptions.Compiled);
            sql = regex.Replace(sql, "$1 [$2$]");
