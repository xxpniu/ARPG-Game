...
namespace DapperQ.Factory
{
    internal class SqlStatementFactory
    {
        internal static SqlStatement Create(DbType type)
        {
            switch (type)
            {
                case DbType.Mysql:
                    return (SqlStatement)new MySql();
                case DbType.SqlServer:
                    return (SqlStatement)new SqlServer();
                case DbType.OleDb:
                    return (SqlStatement)new Access();
                case DbType.Excel:
                    return (SqlStatement)new Excel();
                case DbType.Oracle:
                    return (SqlStatement)new Oracle();
                case DbType.PostgreSQL:
                    return (SqlStatement)new PostgreSQL();
                default:
                    return (SqlStatement)new SqLite();
            }
        }
    }
}
